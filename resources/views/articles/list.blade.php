                        ...
                        @foreach($articles as $article)
                            <tr class="border-b">
                                ...
                                <td class="px-6 py-3 text-center">
                                    @can('edit articles')
                                        <a href="{{ route('articles.edit',$article->id) }}" class="hover:bg-slate-600 bg-slate-700 text-sm rounded-md text-white px-3 py-3">Edit</a>
                                    @endcan
                                    @can('delete articles')
                                        <a href="javascript:void(0)" onclick="deleteArticle('{{$article->id}}')" class="hover:bg-red-500 bg-red-600 text-sm rounded-md text-white px-3 py-3">Delete</a>
                                    @endcan    
                            </td>
                            </tr>  
                        @endforeach
